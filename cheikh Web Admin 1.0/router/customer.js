...
router.post("/add_customer", auth, async(req, res)=>{
    try {
        ...
        if (await DataInsert(`tbl_customer`,
            `profile_image, name, email, country_code, phone, password, status, referral_code, wallet`,

            `'', ${esname}, '${email}', '${country_code}', '${phone}', '${hash}', '${statuss}', '${otp_result}', '0'`, req.hostname, req.protocol) == -1) {
