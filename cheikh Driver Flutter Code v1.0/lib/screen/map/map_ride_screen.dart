...
import 'package:zippygo_driver/screen/auth_screen/splash_screen.dart';
import 'package:zippygo_driver/screen/home/home_screen.dart';
import 'package:zippygo_driver/widget/common.dart';
...
class MapRideScreen extends StatefulWidget {
  ...
  const MapRideScreen({super.key, required this.time, required this.requestId, this.timeStatus});
...
class _MapRideScreenState extends State<MapRideScreen> {
  ...
  mapThemeStyle(){
    if(darkMode == true){
      setState(() {
        DefaultAssetBundle.of(context).loadString("i_theme/dark_theme.json").then((value) {
          setState(() {
            themeForMap = value;
          });
        },);
    ...
    timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      setState(() {
        if (remainingTime > 0) {
          ...
          timer.cancel();
          requestDetailController.requestDetailModel!.requestData.status == "2" ? startTimerAdd() :  timer.cancel();
        }
      });
    });
  }
    ...
    timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      setState(() {
        if (remainingTime2 > 0) {
          ...
          timer.cancel();
          requestDetailController.requestDetailModel!.requestData.status == "2"  ? startTimerAdd1() :  timer.cancel();
        }
      });
    });
  }
  ...
  socketConnect() async {
    ...
    requestDetailController.requestDetailApi(requestId: widget.requestId).then((value) {
      ...
      if(mapData["request_data"]["status"] == "1"){
        ...
        print("++++++++++++++++latitude+++++++++++++++++++++ ${pickLatLon["latitude"]}");
        print("++++++++++++++longitude++++++++++++++ ${pickLatLon["longitude"]}");
        ...
        setState(() {});
      }else if(mapData["request_data"]["status"] == "5" || mapData["request_data"]["status"] == "6"){
  ...
  _connectSocket() {
    ...
    socket.on("Vehicle_Ride_Cancel", (status){
      print("++++++status++++++++++++++++ ${status}");
      if (status["driverid"].toString().contains(getData.read("UserLogin")["id"].toString())) {
    ...
    socket.on("Vehicle_Time_Request${getData.read("UserLogin")['id']}", (data){
      bottomSheetTime(requestID: widget.requestId.toString());
    });
  ...
  RequestDetailController requestDetailController = Get.put(RequestDetailController());
  ...
  MapLocationUpdateController mapLocationUpdateController = Get.put(MapLocationUpdateController());
  ...
  CheckVehicleRequestController checkVehicleRequestController = Get.put(CheckVehicleRequestController());
  ...
  CancelRequestController cancelRequestController = Get.put(CancelRequestController());
  CancelRequestReasonController cancelRequestReasonController = Get.put(CancelRequestReasonController());
        ...
        leading: GetBuilder<RequestDetailController>(
          builder: (context) {
            return requestDetailController.isLoading
                ? requestDetailController.requestDetailModel!.requestData.status == "1" || requestDetailController.requestDetailModel!.requestData.status == "2" || requestDetailController.requestDetailModel!.requestData
                .status == "3" ||
                requestDetailController.requestDetailModel!.requestData
                    .status == "5" ||
                requestDetailController.requestDetailModel!.requestData
                    .status == "6" ||
                requestDetailController.requestDetailModel!.requestData
            ...
            ? GestureDetector(
              onTap: () {
                checkVehicleRequestController.checkVehicleApi(uid: getData.read("UserLogin")["id"].toString()).then((value) {
                  if (requestDetailController.requestDetailModel!.requestData.status == "1" || requestDetailController.requestDetailModel!.requestData
                      .status == "2" ||
                      requestDetailController.requestDetailModel!.requestData
                          .status == "3" ||
                      requestDetailController.requestDetailModel!.requestData
                ...
                child: Icon(Icons.arrow_back,size: 20,color: notifier.textColor))
                : const SizedBox()
                : const SizedBox();
          }
        ),
                ...
                const Spacer(),
                 ...
                 GestureDetector(
                   ...
                   child: Container(
                     ...
                     child: Icon(Icons.timer_rounded,size: 18.5,color: appColor),
                   ),
                 ),
      ...
      body: GetBuilder<RequestDetailController>(
        builder: (requestDetailController) {
          return requestDetailController.isLoading
              ...
              onWillPop: () async {
            bool shouldPop = true;
            await checkVehicleRequestController.checkVehicleApi(uid: getData.read("UserLogin")["id"].toString()).then((value) {

              if (requestDetailController.requestDetailModel!.requestData.status == "2" || requestDetailController.requestDetailModel!.requestData.status == "3" ||
                  requestDetailController.requestDetailModel!.requestData.status == "5" ||
                  requestDetailController.requestDetailModel!.requestData.status == "6" ||
                  requestDetailController.requestDetailModel!.requestData.status == "7") {
                currentIndexBottom = 0;
                homeStatus = 1;
                setState((){ });
                print('************homeStatus_1--------------- $homeStatus');
                Get.offAll(const BottomBarScreen());
                shouldPop = false;
              }
            });
            return shouldPop;
                ...
                child: Stack(
                  ...
                  children: [
                    GoogleMap(
                      initialCameraPosition:  CameraPosition(target: LatLng(movingLat ?? lat, movingLong ?? long), zoom: 16,),
                      ...
                      markers: Set<Marker>.of(mapLocationUpdateController.markers11.values),
                      polylines: Set<Polyline>.of(mapLocationUpdateController.polylines11.values),
                    ),
                    Container(
                      width: Get.width,
                       ...
                       title: Padding(
                         ...
                         child: Text(
                           requestDetailController.requestDetailModel!.requestData.status == "2"
                               ?  "Enter OTP"
                               :  requestDetailController.requestDetailModel!.requestData.status == "3" || requestDetailController.requestDetailModel!.requestData.status == "6"
                               ?  "Start the ride"
                               :  requestDetailController.requestDetailModel!.requestData.status == "5"
                               ?  "End the ride"
                               :  "Drive to pickup",
                        ...
                        subtitle: Text("Please don't be late",style: TextStyle(
                          color: notifier.textColor,
                          fontSize: 16,
                          letterSpacing: 0.5,
                        ...
                        trailing: Row(
                          ...
                          children: [
                            Text(
                              homeStatus == 0 ? formatTime(remainingTime) : formatTime2(remainingTime2),
                              style: TextStyle(
                              color: appColor,
                              fontSize: 25,
                              letterSpacing: 0.5,
                    ...
                    Positioned(
                      ...
                      child: GestureDetector(
                        onTap: () async {
                          ...
                          // ));
                          await launchUrl(Uri.parse('google.navigation:q=${double.parse(requestDetailController.requestDetailModel!.requestData.dropLatlon[0].latitude)}, ${double.parse(requestDetailController.requestDetailModel!.requestData.dropLatlon[0].longitude)}&key="${Config.mapKey}"'));
                        },
                        child: Container(
                          height: 40,
                          ...
                          child: Row(
                            ...
                            children: [
                              SizedBox(
                                  height: 22,
                                  child: Image.asset("assets/image/location-arrow.png",color: whiteColor,)),
                                     ...
                                     children: [
                                       Column(
                                         ...
                                         children: [
                                               ...
                                               Column(
                                                 ...
                                                 children: [
                                                   Container(
                                                     ...
                                                     child:  Text(
                                                       requestDetailController.requestDetailModel!.requestData.name[0].toUpperCase(),
                                                       style: TextStyle(
                                                         color: whiteColor,
                                                         fontSize: 18,
                                                         fontFamily: FontFamily.sofiaProBold,
                                                   ...
                                                   Text(
                                                     requestDetailController.requestDetailModel!.requestData.name,
                                                     style: TextStyle(
                                                       color: notifier.textColor,
                                                       fontSize: 16,
                                                       fontFamily:
                                                 ...
                                                 child: Column(
                                                   ...
                                                   children: [
                                                     ...
                                                     ListView.separated(
                                                       ...
                                                       itemCount: requestDetailController.requestDetailModel!.requestData.dropAdd.length,
                                                         ...
                                                         return Row(
                                                           ...
                                                           children: [
                                                             SizedBox(
                                                                 height: 20,
                                                                 width: 20,
                                                                 child: SvgPicture.asset("assets/image/loaction_circle.svg")),
                                                             const SizedBox(width: 6),
                                                             Flexible(
                                                               child: Text(
                                                     ...
                                                     Row(
                                                       ...
                                                       children: [
                                                         ...
                                                         GestureDetector(
                                                           onTap: () {
                                                             setState(() {
                                                               _makingPhoneCall(number: "${requestDetailController.requestDetailModel!.requestData.countryCode}${requestDetailController.requestDetailModel!.requestData.phone}");
                                                             });
                                                           },
                                                           child: Container(
                                                             alignment: Alignment.center,
                                                             ...
                                                             child: Icon(Icons.call,size: 23,color: appColor),
                                                           ),
                                                         ),
                                                         const SizedBox(width: 10),
                                                         GestureDetector(
                                                           onTap: (){
                                                             Get.to(ChatScreen(customer: requestDetailController.requestDetailModel!.requestData.cId.toString(),));
                                                           },
                                                           child: Container(
                                                             alignment: Alignment.center,
                                                             height: 50,
                                                             ...
                                                             child: Icon(CupertinoIcons.chat_bubble_2_fill,size: 23,color: appColor),
                                                           ),
                                                         ),
                                                         const SizedBox(width: 10),
                                           ...
                                           const SizedBox(height: 20),
                                           requestDetailController.requestDetailModel!.requestData.status == "1"
                                               ? button(text: "I'm here", color: appColor,
                                             onPress: (){
                                               homeStatus = 0;
                                               mapLocationUpdateController.dropOffPoints = [];
                                               mapLocationUpdateController.markers11 = {};
                                               iAmHereController.iAmHereApi(context: context, requestID: widget.requestId.toString()).then((value) {
                                                 Map<String, dynamic> decodedValue = json.decode(value);

                                                 if(decodedValue["Result"] == true){
                                                   iAmHereTime = decodedValue["driver_wait_time"];
                                                   countdownStart = (int.parse(iAmHereTime.toString()) * 60);
                                                   remainingTime = countdownStart; // Initialize remainingTime
                                                   startTimer();
                                                   print("++++++++++iAmHereTime++++++++++++++++ ${iAmHereTime}");
                                                   socket.emit('Vehicle_D_IAmHere',{
                                                     ...
                                                     'pickuptime' : decodedValue["driver_wait_time"],
                                                   });
                                                   requestDetailController.requestDetailApi(requestId: widget.requestId).then((value) {
                                                     Map<String, dynamic> mapData = json.decode(value);
                                                     print("++++++++++++++++ ${mapData}");
                                               ...
                                               :  requestDetailController.requestDetailModel!.requestData.status == "2"
                                               ? button(text: "Enter OTP", color: appColor,
                                               onPress: (){
                                                 setState(() {
                                                   otpBottomSheet();
                                                 });
                                           ...
                                           ) : requestDetailController.requestDetailModel!.requestData.status == "5" ? button(text: "Finish the ride", color: Colors.red,
                                               onPress: (){
                                           ...
                                           // ) : requestDetailController.requestDetailModel!.requestData.status == "7" ? button(text: "All ride complete", color: Colors.red,
                                           //     onPress: (){
                                           //   Get.offAll(RideCompleteScreen(requestId: widget.requestId.toString(),cID: requestDetailController.requestDetailModel!.requestData.cId.toString()));
                                           //     }
                                           )
                                               : const SizedBox(),
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Padding(
                    padding: const EdgeInsets.all(12),
                    child: button(text: "Send", color: appColor,
                        onPress: (){
                          otpRideController.otpRideApi(context: context, requestId: widget.requestId.toString(), otp: otpRideController.otpController.text,time: "${timmer == 0 ? "0" : timmer}").then((value) {
                            Map<String, dynamic> decodedValue = json.decode(value);
                            if(decodedValue["Result"] == true){
                              timer?.cancel();
                              homeStatus == 0 ? formatTime(remainingTime) : formatTime2(remainingTime2);
                              print("++++++++remainingTime123+++++++++++ $remainingTime");
                              ...
                              requestDetailController.requestDetailApi(requestId: widget.requestId.toString());
                              Get.back();
                              setState((){});
                            }
                          },);
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: Get.height * 0.02),
                    ListView.builder(
                      itemCount: cancelRequestReasonController.cancelRequestReasonModel!.rideCancelList.length,
                      shrinkWrap: true,
                      physics: const NeverScrollableScrollPhysics(),
                      itemBuilder: (ctx, i) {
                        return InkWell(
                          onTap: () {
                            setState(() {});
                            selectedRadioTile = i;
                            rejectMsg = cancelRequestReasonController.cancelRequestReasonModel!.rideCancelList[i].id.toString();
                          },
                          child: SizedBox(
                            height: 40,
                            child: Row(
                              children: [
                                ...
                                Text(
                                  cancelRequestReasonController.cancelRequestReasonModel!.rideCancelList[i].title,
                                  style: TextStyle(
                                    fontSize: 16,
                                    fontFamily: 'Gilroy Medium',
                                    color: notifier.textColor,
  ...
  cancelButton({Function()? ontap, String? title, Color? bgColor, titleColor, Gradient? gradient1}) {
    return InkWell(
      onTap: ontap,
      child: Container(
  ...
  cancelButton1({Function()? ontap, String? title, Color? bgColor, titleColor, Gradient? gradient1}) {
    return InkWell(
      onTap: ontap,
      child: Container(
        height: Get.height * 0.04,
              ...
              children: [
                Column(
                  ...
                  children: [
                    ...
                    Text("How soon will you arrive?",style: TextStyle(
                      color: notifier.textColor,
                      fontSize: 20,
                      fontFamily:
                      FontFamily.sofiaProBold,
                      ...
                      itemBuilder: (context, index) {
                        return button2(
                            ...
                            onPress: (){
                              ...
                              countdownStart = (int.parse(timeData[index].toString()) * 60); // Initialize countdownStart with widget.time
                    ...
                    button(text: "Cancel", color: appColor,onPress: (){
                      Get.back();
                    }),
                ...
                children: [
                  ...
                  Expanded(
                    child: ListView.separated(
                      ...
                      shrinkWrap: true,
                      itemCount: requestDetailController.requestDetailModel!.demList.length,
                      separatorBuilder: (context, index) => const SizedBox(height: 8),
                      itemBuilder: (context, index) {
                        return button3(textColor: selectNotification == index ? whiteColor : notifier.textColor, borderColor: selectNotification == index ? appColor : notifier.borderColor, text: requestDetailController.requestDetailModel!.demList[index].title, color: selectNotification == index ? appColor: notifier.containerColor,
                          onPress: (){
                          setState((){
                            selectNotification = index;
                            notificationController.notificationId = requestDetailController.requestDetailModel!.demList[index].id.toString();
                          });
                          }
                        );
                      },),
                  ...
                  const SizedBox(height: 10),
                  button(text: "SEND".tr, color: appColor,onPress: (){
                    notificationController.notification(context: context,requestId: widget.requestId.toString());
                  }),
                ],
              ),
            ),
