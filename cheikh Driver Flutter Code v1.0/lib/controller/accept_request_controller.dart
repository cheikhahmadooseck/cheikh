...
class AcceptRequestController extends GetxController implements GetxService {
  ...
  Future acceptRequestApi({required context,required String requestId}) async{
    ...
    var response = await http.post(Uri.parse(Config.baseUrl + Config.acceptRequest),body: jsonEncode(body),headers: userHeader);
    ...
    if (response.statusCode == 200) {
      if (data["Result"] == true) {
        ...
        if (acceptRequestModel!.result == true) {
          ...
          snackBar(context: context, text: acceptRequestModel!.message.toString());
        }
      } else {
