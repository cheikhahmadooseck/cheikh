...
import 'package:zippygo_driver/main.dart';
...
class LoginController extends GetxController implements GetxService{
  ...
  Future loginApi({required context,required String cCode,required String phone, required String password}) async{
    ...
    var response = await http.post(Uri.parse(Config.baseUrl + Config.login),body: jsonEncode(body),headers: userHeader);
    ...
    if(response.statusCode == 200){
      if(data["Result"] == true){
        ...
        if(loginModel!.result == true){
          ...
          log("++++++++++++++++++++++++++++++++++++++++ ${getData.read("Currency")}");
          Config.oneSignal = "${getData.read("OneSignal")}";
          initPlatformState();
          OneSignal.User.addTags({"subscription_user_Type": 'driver', "Login_ID": getData.read("UserLogin")["id"].toString()});
