...
// import 'package:zippygo_driver/controller/verification_check_controller.dart';
...
class AddDocumentController extends GetxController implements GetxService {
  ...
  Future addDocumentApi({required context, required String docId, required String fname, String? frontImage, String? backImage,}) async {
    ...
    if (frontImage != null && frontImage.isNotEmpty) {
      ...
      double frontFileSizeInMB = frontFileSizeInBytes / (1024 * 1024); // Convert bytes to MB
      if (frontFileSizeInMB > 1) {
        snackBar(context: context, text: "Image size exceeds 1 MB. Please select a smaller file.");
        return;
      }
    }
    ...
    if (backImage != null && backImage.isNotEmpty) {
      ...
      double backFileSizeInMB = backFileSizeInBytes / (1024 * 1024); // Convert bytes to MB
