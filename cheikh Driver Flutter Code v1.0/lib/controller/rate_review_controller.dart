...
class RateReviewController extends GetxController implements GetxService {
  ...
  Future rateReviewAPi({required context,required String requestId, required String cID}) async{
    ...
    var response = await http.post(Uri.parse(Config.baseUrl + Config.rateReview),body: jsonEncode(body),headers: userHeader);
    ...
    if (response.statusCode == 200) {
      if (data["Result"] == true) {
        ...
        if (rateAndReviewModel!.result == true) {
          ...
          snackBar(context: context, text: rateAndReviewModel!.message.toString());
        }
      } else {
        snackBar(context: context, text: "${data["message"]}");
      }
