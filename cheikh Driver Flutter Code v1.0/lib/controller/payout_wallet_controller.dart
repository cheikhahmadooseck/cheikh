...
import 'package:zippygo_driver/controller/wallet_controller.dart';
...
class PayoutWalletController extends GetxController implements GetxService {
  ...
  payoutApi({required context}) async {
    ...
    var response = await http.post(Uri.parse(Config.baseUrl + Config.payout),body: jsonEncode(body),headers: userHeader);
    ...
    if(response.statusCode == 200){
      if(data["Result"] == true){
        ...
        if(payoutWalletModel!.result == true){
          ...
          snackBar(context: context, text: payoutWalletModel!.message.toString());
          update();
