...
class CancelRequestController extends GetxController implements GetxService {
  ...
  Future cancelRequestApi({required context,required String requestId,required String cID,String? cancelId}) async{
    ...
    var response = await http.post(Uri.parse(Config.baseUrl + Config.cancelRequest),body: jsonEncode(body),headers: userHeader);
    ...
    if (response.statusCode == 200) {
      if (data["Result"] == true) {
        ...
        if (cancelRequestModel!.result == true) {
          ...
          snackBar(context: context, text: cancelRequestModel!.message.toString());
        }
      } else {
