...
import 'package:zippygo/api_code/vihical_calculate_api_controller.dart';
...
class HomeScreen extends StatefulWidget {
  ...
  const HomeScreen({super.key, required this.latpic, required this.longpic, required this.latdrop, required this.longdrop, required this.destinationlat});
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  _connectSocket() async {
    ...
    socket.onConnect((data) => print('Connection established Connected home screen'));
    socket.onConnectError((data) => print('Connect Error home screen: $data'));
    socket.onDisconnect((data) => print('Socket.IO server disconnected home screen'));

    print("*********midsecounde*********:--  ($midseconde)");
    print("*********midsecounde*********:--  ($vihicalrice)");
    vihicalCalculateController.vihicalcalculateApi(uid: useridgloable.toString(), mid: "$midseconde", pickup_lat_lon: "$latitudepick,$longitudepick", drop_lat_lon: "$latitudedrop,$longitudedrop", drop_lat_lon_list: onlypass).then((value) {
      print("-----------------:--- $value");
      // print("-----------------:--- ${vihicalCalculateController.vihicalCalculateModel!.caldriver!.length}");
      for(int i=0; i<vihicalCalculateController.vihicalCalculateModel!.caldriver!.length;i++){
        vihicallocationsbiddingoff.add(LatLng(double.parse(vihicalCalculateController.vihicalCalculateModel!.caldriver![i].latitude!), double.parse(vihicalCalculateController.vihicalCalculateModel!.caldriver![i].longitude!)));
        _iconPathsbiddingoff.add("${Config.imageurl}${vihicalCalculateController.vihicalCalculateModel!.caldriver![i].image}");
      }
      _addMarkers();
    },);
    ...
    socket.on('acceptvehrequest$useridgloable', (acceptvehrequest) {
      ...
      print("++++hjhhhhhhhhhhhhhhhhhh+++++: ${acceptvehrequest["uid"].toString()}");
      print("++++hjhhhhhhhhhhhhhhhhhh+++++: ${acceptvehrequest["uid"].toString()}");
      loadertimer = true;
      ...
      if(acceptvehrequest["c_id"].toString().contains(useridgloable.toString())){
        print("condition done");
        driveridloader == false;
        print("condition done1");
        print("condition done0 ${context}");
        globalDriverAcceptClass.driverdetailfunction(context: context,lat: widget.latpic,long: widget.longpic,d_id: acceptvehrequest["uid"].toString(), request_id: acceptvehrequest["request_id"].toString());
  ...
  void initState() {
    ...
    /// origin marker
    _addMarker(LatLng(widget.latpic, widget.longpic), "origin", BitmapDescriptor.defaultMarker);
    ...
    getDirections(lat1: PointLatLng(widget.latpic, widget.longpic),lat2: PointLatLng(widget.latdrop, widget.longdrop),dropOffPoints: _dropOffPoints);
  ...
  Future getDirections({required PointLatLng lat1,required PointLatLng lat2,required List<PointLatLng> dropOffPoints}) async {
    List<LatLng> polylineCoordinates = [];
  ...
  bool light = true;
   ...
   CancelRasonRequestApiController cancelRasonRequestApiController = Get.put(CancelRasonRequestApiController());
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: GetBuilder<Modual_CalculateController>(builder: (modual_calculateController) {
        return modual_calculateController.isLoading ? Center(child: CircularProgressIndicator(color: theamcolore)) : Stack(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: ListView.separated(
                          ...
                          itemCount: modual_calculateController.modualCalculateApiModel!.caldriver!.length,
                            ...
                            return InkWell(
                              onTap: () {
                                setState(() {
                                  ...
                                  vihicallocationsbiddingoff.clear();
                                  _iconPathsbiddingoff.clear();
                                  print("++++vihicallocationsbiddingoff+++++:-- $vihicallocationsbiddingoff");
                                  print("+++++_iconPathsbiddingoff++++++++++:-- $_iconPathsbiddingoff");
                                  setState(() {});

                                  midseconde = modual_calculateController.modualCalculateApiModel!.caldriver![index].id!;
                                  vihicalrice = double.parse(modual_calculateController.modualCalculateApiModel!.caldriver![index].dropPrice!.toString());
                                  totalkm = double.parse(modual_calculateController.modualCalculateApiModel!.caldriver![index].dropKm!.toString());
                                  tot_time = modual_calculateController.modualCalculateApiModel!.caldriver![index].dropTime!.toString();
                                  tot_hour = modual_calculateController.modualCalculateApiModel!.caldriver![index].dropHour!.toString();
                                  tot_secound = "0";
                                  vihicalname = modual_calculateController.modualCalculateApiModel!.caldriver![index].name!.toString();
                                  vihicalimage = modual_calculateController.modualCalculateApiModel!.caldriver![index].image!.toString();
                                  vehicle_id = modual_calculateController.modualCalculateApiModel!.caldriver![index].id!.toString();

                                  setState(() {

                                  });
                                  print("+++++dropprice++++++++++:-- $vihicalrice");

                                  vihicalCalculateController.vihicalcalculateApi(uid: useridgloable.toString(), mid: midseconde.toString(), pickup_lat_lon: "$latitudepick,$longitudepick", drop_lat_lon: "$latitudedrop,$longitudedrop", drop_lat_lon_list: onlypass).then((value) {
                                    setState(() {
                                ...
                                child: Center(
                                  child: ListTile(
                                    leading: Container(
                                        ...
                                        child: Image.network("${Config.imageurl}${modual_calculateController.modualCalculateApiModel!.caldriver![index].image}"),
                                    ),
                                    title: Row(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      mainAxisAlignment: MainAxisAlignment.start,
                                      children: [
                                        Text('${modual_calculateController.modualCalculateApiModel!.caldriver![index].name}',style: TextStyle(color: notifier.textColor,fontFamily: "SofiaProBold",fontSize: 16)),
                                        const SizedBox(width: 10,),
                                        select == index ? SvgPicture.asset("assets/svgpicture/user.svg",height: 15,color: notifier.textColor,) : const SizedBox(),
                                        select == index ? Text("${modual_calculateController.modualCalculateApiModel!.caldriver![index].passengerCapacity}",style: TextStyle(color: notifier.textColor),) : const SizedBox(),
                                      ],
                                    ),
                                    subtitle: Padding(
                                      padding: const EdgeInsets.only(top: 8.0),
                                      child: Text('${modual_calculateController.modualCalculateApiModel!.caldriver![index].driPicTime} min away - Drop ${modual_calculateController.modualCalculateApiModel!.caldriver![index].driPicDrop}',style: const TextStyle(color: Colors.grey,fontSize: 14)),
                                    ),
                                    trailing: Text("$currencyy${modual_calculateController.modualCalculateApiModel!.caldriver![index].dropPrice}",style: TextStyle(color: notifier.textColor,fontSize: 16)),
                                  ),
                                ),
                              ),
                            );
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                                                  ...
                                                  children: <Widget> [
                                                    ...
                                                    walleteamount == 0 ? const SizedBox() : Row(
                                                      children: [
                                                        ...
                                                        const SizedBox(width: 10,),
                                                        Text("My Wallet ($currencyy$walleteamount)",style: TextStyle(color: notifier.textColor),),
                                                        const Spacer(),
                                                        Transform.scale(
                                                          scale: 0.8,
                          ...
                          return Expanded(
                            child: InkWell(
                                                                  ...
                                                                  children: [
                                                                    ...
                                                                    Row(
                                                                      children: [
                                                                        ...
                                                                        Text("${currencyy}${paymentGetApiController.paymentgetwayapi!.couponList![index].discountAmount}",style: TextStyle(fontWeight: FontWeight.bold,color: notifier.textColor),),
                                                                      ],
                                                                    ),
                                                                    const SizedBox(height: 5,),
                                                                    Row(
                                                                      children: [
                                                                        ...
                                                                        Text("${currencyy}${paymentGetApiController.paymentgetwayapi!.couponList![index].minAmount}",style: TextStyle(fontWeight: FontWeight.bold,color: notifier.textColor),),
                                                                      ],
                                                                    ),
                                                                    const SizedBox(height: 5,),
                                                                    Row(
                                                                    ...
                                                                    couponadd[index] == true ? InkWell(
                                                                        ...
                                                                        child: Container(
                                                                            ...
                                                                            child: Center(child: Text("Remove".tr,style: const TextStyle(color: Colors.red),)))) : InkWell(
                                                                      onTap: () {
                                                                      ...
                                                                      child: Container(
                                                                          ...
                                                                          child: Center(child: Text("Apply coupons".tr,style: TextStyle(color: double.parse(paymentGetApiController.paymentgetwayapi!.couponList![index].minAmount!) < double.parse(mainamount) ? theamcolore : Colors.grey),))
                                                                      ),
                                                                    ),
                                                                  ],
                                                                ),
                              ...
                              child: ListTile(
                                ...
                                title: couponname == "" ? Transform.translate(offset: Offset(-15, 10),child: Text("Coupon".tr,style: TextStyle(color: notifier.textColor),)) : Transform.translate(offset: const Offset(-15, 0),child: Text("$couponname",style: TextStyle(color: notifier.textColor),)),
                    ...
                    select == -1 ? CommonButton(containcolore: theamcolore.withOpacity(0.1), onPressed1: () {},context: context,txt1: "Book $vihicalname")  : CommonButton(containcolore: theamcolore, onPressed1: () {
                      // socket.close();


                      homeWalletApiController.homwwalleteApi(uid: useridgloable.toString(),context: context).then((value) {
                        print("{{{{{[wallete}}}}}]:-- ${value["wallet_amount"]}");
                        walleteamount = double.parse(value["wallet_amount"]);
                        print("[[[[[[[[[[[[[walleteamount]]]]]]]]]]]]]:-- ($walleteamount)");
                      },);

                      print("pickupadd title:- ${picktitle == "" ? addresspickup : picktitle}");
                      print("pickupadd sub :- $picksubtitle");
                      print("dropadd title :- $droptitle");
                      print("dropadd sub :- $dropsubtitle");
                      print("list sub :- $droptitlelist");
                      print("99999999999999999999999999 :- ${vihicalCalculateController.vihicalCalculateModel!.driverId}");
                      ...
                      commonbottomsheetrequestsend(context: context);
                    },context: context,txt1: "Book $vihicalname")
                  ],
                ),
              ),
            ),
