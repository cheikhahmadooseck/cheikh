...
import 'package:zippygo/app_screen/driver_detail_screen.dart';
...
class _DriverStartrideScreenState extends State<DriverStartrideScreen> {
  ...
  _addMarker3(String id) async {
    for (int a = 0; a < droppointstartscreen.length; a++) {
      ...
      LatLng position = LatLng(droppointstartscreen[a].latitude, droppointstartscreen[a].longitude);
  ...
  _connectSocket() async {
    ...
    socket.on('V_Driver_Location$useridgloable', (V_Driver_Location) {
      ...
      if(driver_id == V_Driver_Location["d_id"].toString()){
        ...
        if(droppointstartscreen.isEmpty){
          ...
          getDirections(lat1: PointLatLng(livelat, livelong),dropOffPoints: droppointstartscreen);
          print("==========:--- ${droppointstartscreen}");
    ...
    socket.on('drop_location$useridgloable', (drop_location) {
      ...
      imagenetwork = "${Config.imageurl}${drop_location["driver_location"]["image"]}";
      ...
      for(int i=0; i<listdrop.length; i++){
        droppointstartscreen.add(PointLatLng(double.parse(drop_location["drop_list"][i]["latitude"]), double.parse(drop_location["drop_list"][i]["longitude"])));
      }
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                ...
                Row(
                  children: [
                    Text("Heading to the destination".tr,style: TextStyle(fontSize: 16,color: notifier.textColor),),
                    const Spacer(),
                    // Container(
                    //   height: 40,
                    //   width: 80,
                    ...
                    // SizedBox(width: 5,),
                    timervarable == false ? SizedBox() :  TimerScreen(hours: int.parse(totaldrophour),minutes: int.parse(totaldropmint),secound: int.parse(totaldropsecound),),
                  ],
                ),
                const SizedBox(height: 10,),
                ...
                ListTile(
                  ...
                  title: Text("Drop to".tr,style: TextStyle(color: notifier.textColor),),
                  // subtitle: Text("${droptitle}"),
                  subtitle: listdrop.isEmpty ? SizedBox() : Text("${listdrop[0]["title"]}",style: TextStyle(color: notifier.textColor),),
                      ...
                      children: [
                        ListTile(
                          ...
                          title: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(drivername,style: TextStyle(color: notifier.textColor,fontSize: 18)),
                              const SizedBox(height: 2,),
                              Text(drivervihicalnumber,style: const TextStyle(color: Colors.grey,fontSize: 12),),
                              const SizedBox(height: 5,),
                            ],
                          ),
                          subtitle: Text(driverlanguage,style: TextStyle(color: notifier.textColor),),
                          trailing: Stack(
                                ...
                                child: Container(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      ...
                                      const SizedBox(width: 10,),
                                      Text("Message ${drivername}",style: TextStyle(color: notifier.textColor),)
                                    ],
                                  ),
                                ),
                              ),
        ...
        // ),
        body: droppointstartscreen.isEmpty ? SizedBox() : Stack(
          children: [
            GoogleMap(
              initialCameraPosition: CameraPosition(target: LatLng(livelat, livelong),zoom: 15),
              myLocationEnabled: true,
