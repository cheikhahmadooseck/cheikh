...
import 'package:zippygo/api_code/profile_edit_api_controller.dart';
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  ProfileeditApiController profileeditApiController = Get.put(ProfileeditApiController());
      ...
      bottomNavigationBar: Padding(
        ...
        child: CommonButton(containcolore: theamcolore, onPressed1: () {
          ...
          if(namecontroller.text.isNotEmpty && emailcontroller.text.isNotEmpty){
            profileeditApiController.PrifileediteApi(profile_img: _selectedprofile == null ? "" : _selectedprofile!.path,id: decodeUid["id"].toString(), name: namecontroller.text, email: emailcontroller.text, password: passwordcontroller.text).then((value) async {

              // SharedPreferences pre = await SharedPreferences.getInstance();
              // pre.setString("loginData", jsonEncode(value["UserLogin"]));
              SharedPreferences preferences = await SharedPreferences.getInstance();
              preferences.setString("userLogin", jsonEncode(value["customer_data"]));
              getlocledata().then((value) async{
        ...
        },context: context,txt1: "Update".tr),
      ),
      appBar: AppBar(
        backgroundColor: notifier.background,
        elevation: 0,
        ...
        centerTitle: true,
        title:  Text("My Profile".tr,style: TextStyle(color: notifier.textColor),),
      ),
      body: loading == true ? Center(child: CircularProgressIndicator(color: theamcolore,)) : Padding(
        ...
        child: Column(
          children: [
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: _selectedprofile != null
                        ? ClipRRect(borderRadius: BorderRadius.circular(65), child: Image.file(_selectedprofile!, fit: BoxFit.cover,))
                        : decodeUid["profile_image"] == "" ? Center(child: Text("${decodeUid["name"][0]}",style: const TextStyle(color: Colors.white,fontSize: 25))) :
                        ClipRRect(borderRadius: BorderRadius.circular(65), child: Image.network("${Config.imageurl}${decodeUid["profile_image"]}",fit: BoxFit.cover,)),
                  ),
            ...
            Text(decodeUid["name"] == "" ? "" : decodeUid["name"],style: TextStyle(color: notifier.textColor,fontWeight: FontWeight.bold,fontSize: 20),),
            const SizedBox(height: 30,),
            CommonTextfiled200(txt: "Name".tr, context: context,controller: namecontroller),
            const SizedBox(height: 10,),
            CommonTextfiled200(txt: "Email".tr, context: context,controller: emailcontroller),
            const SizedBox(height: 10,),
