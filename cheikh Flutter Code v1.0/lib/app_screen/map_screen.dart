...
import 'package:zippygo/api_code/coupon_payment_api_contoller.dart';
import 'package:zippygo/api_code/home_controller.dart';
import 'package:zippygo/app_screen/home_screen.dart';
import 'package:zippygo/app_screen/pagelist_description.dart';
import 'package:zippygo/app_screen/pickup_drop_point.dart';
import 'package:zippygo/app_screen/profile_screen.dart';
import 'package:zippygo/app_screen/refer_and_earn.dart';
import 'package:zippygo/app_screen/top_up_screen.dart';
import 'dart:ui' as ui;
import 'package:zippygo/common_code/colore_screen.dart';
import 'package:zippygo/common_code/common_button.dart';
import 'package:zippygo/common_code/common_flow_screen.dart';
...
class _MapScreenState extends State<MapScreen> with TickerProviderStateMixin{
  ...
  ResendRequestApiController resendRequestApiController = Get.put(ResendRequestApiController());
  RemoveRequest removeRequest = Get.put(RemoveRequest());
  TimeoutRequestApiController timeoutRequestApiController = Get.put(TimeoutRequestApiController());
  ...
  HomeWalletApiController homeWalletApiController = Get.put(HomeWalletApiController());
  AddVihicalCalculateController addVihicalCalculateController = Get.put(AddVihicalCalculateController());
  VihicalInformationApiController vihicalInformationApiController = Get.put(VihicalInformationApiController());
  VihicalCalculateController vihicalCalculateController = Get.put(VihicalCalculateController());
  GlobalDriverAcceptClass globalDriverAcceptClass = Get.put(GlobalDriverAcceptClass());
  ...
  _connectSocket() async {
    ...
    SharedPreferences preferences = await SharedPreferences.getInstance();
    socket.onConnect((data) => print('Connection established Connected map screen'));
    socket.onConnectError((data) => print('Connect Error11111 map screen: $data'));
    socket.onDisconnect((data) => print('Socket.IO server disconnected map screen'));

    homeWalletApiController.homwwalleteApi(uid: userid.toString(),context: context).then((value) {
    ...
    homeApiController.homeApi(uid: userid.toString(),lat: lathome.toString(),lon: longhome.toString()).then((value) {
      ...
      homeMapController.homemapApi(mid: mid,lat: lathome.toString(), lon: longhome.toString()).then((value) {
        setState(() {
        ...
        if(value["Result"] == false){
          ...
          for(int i=0; i<homeMapController.homeMapApiModel!.list!.length;i++){
            vihicallocations.add(LatLng(double.parse(homeMapController.homeMapApiModel!.list![i].latitude.toString()), double.parse(homeMapController.homeMapApiModel!.list![i].longitude.toString())));
            _iconPaths.add("${Config.imageurl}${homeMapController.homeMapApiModel!.list![i].image}");
          }
          _addMarkers();
      ...
      homeMapController.homemapApi(mid: mid,lat: lathome.toString(), lon: longhome.toString()).then((value) {
        ...
        if(value["Result"] == false){
          ...
          for(int i=0; i<homeMapController.homeMapApiModel!.list!.length;i++){
            vihicallocationsbiddingon.add(LatLng(double.parse(homeMapController.homeMapApiModel!.list![i].latitude.toString()), double.parse(homeMapController.homeMapApiModel!.list![i].longitude.toString())));
            _iconPathsbiddingon.add("${Config.imageurl}${homeMapController.homeMapApiModel!.list![i].image}");
          }
          _addMarkers2();
    ...
    socket.on('home', (messaj) {
      ...
      homeApiController.homeApi(uid: messaj['uid'].toString(), lat: messaj['lat'].toString(), lon: messaj['lon'].toString()).then((value) {
        mid = homeApiController.homeapimodel!.categoryList![0].id.toString();
        mroal = homeApiController.homeapimodel!.categoryList![0].role.toString();
      },);
    });
    ...
    socket.on("Driver_location_On", (Driver_location_On) async {
      ...
      if (zonelist.contains(homeMapController.homeMapApiModel!.zoneId)) {
        ...
        LatLng postion = LatLng(double.parse(Driver_location_On["latitude"]), double.parse(Driver_location_On["longitude"]));
        final Uint8List markIcon = await getNetworkImage("${Config.imageurl}${Driver_location_On["image"]}");
    ...
    socket.on("Driver_location_Update", (Driver_location_Update) async {
      ...
      LatLng postion = LatLng(double.parse(Driver_location_Update["latitude"]), double.parse(Driver_location_Update["longitude"]));
      final Uint8List markIcon = await getNetworkImage("${Config.imageurl}${Driver_location_Update["image"]}");
    ...
    socket.on("Vehicle_Bidding$userid", (Vehicle_Bidding){
      ...
      for(int i =0; i<vehicle_bidding_driver.length; i++){
        vehicle_bidding_secounde.add(Vehicle_Bidding["bidding_list"][i]["diff_second"]);
      }
  ...
  void startTimer() {
    ...
    timer = Timer.periodic(const Duration(seconds: 1), (Timer t) {
      if (durationInSeconds > 0) {
        setState(() {
          durationInSeconds--;
          print("Timer seconds remaining: $durationInSeconds");
        });
      } else {
        t.cancel();
  ...
  requesttime(){
    ...
    durationInSeconds = int.parse(calculateController.calCulateModel!.offerExpireTime.toString());
    ...
    controller.addStatusListener((status) {
      if (status == AnimationStatus.completed) {
        ...
        if (isControllerDisposed) {
          print("Controller has already been disposed. Skipping further actions.");
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
            ...
            Get.back();
            timeoutRequestApiController.timeoutrequestApi(uid: userid.toString(),request_id: request_id.toString()).then((value) {
                      ...
                      child:  Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                SvgPicture.asset("assets/svgpicture/exclamation-circle.svg",height: 25,),
                                const SizedBox(width: 10,),
                                Text("Captains are busy".tr,style: const TextStyle(color: Colors.black,fontWeight: FontWeight.bold,fontSize: 18),),
                              ],
                            ),
                              ...
                              child: Row(
                                ...
                                children: [
                                  Expanded(
                                    flex: 1,
                                    child: Padding(
                                      padding: const EdgeInsets.only(top: 16,bottom: 20,left: 0),
                                      child: Column(
                                        children: [
                                          Container(
                                            height: 15,
                                          ...
                                          const SizedBox(height: 4,),
                                          textfieldlist.isNotEmpty ? const SizedBox() : Container(
                                            height: 10,
                                            width: 3,
                                            decoration: BoxDecoration(
                                                color: Colors.grey.withOpacity(0.4),
                                          ...
                                          const SizedBox(height: 4,),
                                          textfieldlist.isEmpty ? const SizedBox() : Container(
                                            height: 10,
                                            width: 3,
                                            decoration: BoxDecoration(
                                                color: Colors.grey.withOpacity(0.4),
                                  ...
                                  Expanded(
                                    ...
                                    child: Padding(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          Transform.translate(
                                            offset: picktitle == "" ? const Offset(0, 0) : const Offset(0, -10),
                                            child: ListTile(
                                              // isThreeLine: true,
                                              contentPadding: EdgeInsets.zero,
                                              title: Text("${picktitle == "" ? addresspickup : picktitle}"),
                                              subtitle: Text(picksubtitle,style: const TextStyle(color: Colors.grey),maxLines: 1,),
                                            ),
                                          ),
                                          ...
                                          Transform.translate(
                                            ...
                                            child: ListTile(
                                              ...
                                              subtitle: Text(dropsubtitle,style: const TextStyle(color: Colors.grey),maxLines: 1,),
                                            ),
                                          ),
                                        ],
                                      ),
                            ...
                            textfieldlist.isEmpty ? const SizedBox() : Transform.translate(
                              offset: const Offset(0, -30),
                                          ...
                                          child: Column(
                                            children: [
                                              ...
                                              ListTile(
                                                // isThreeLine: true,
                                                contentPadding: EdgeInsets.zero,
                                                title: Text("${droptitlelist[index]["title"]}"),
                                                subtitle: Text("${droptitlelist[index]["subt"]}",style: const TextStyle(color: Colors.grey),maxLines: 1,),
                                              ),
                                            ],
                                          ),
                                        ),
                            ...
                            CommonButton(containcolore: theamcolore, onPressed1: () {
                              ...
                              //   requesttime();
                              // },);
                            },txt1: "Try Again".tr,context: context),
                            const SizedBox(height: 10,),
                            CommonOutLineButton(bordercolore: theamcolore, onPressed1: () {
                              removeRequest.removeApi(uid: userid.toString()).then((value) {
                                Get.back();
                                print("+++ removeApi +++:- ${value["driver_list"]}");
                            ...
                            },txt1: "Cancel".tr,context: context),
                            const SizedBox(height: 10,),
                          ],
                        ),
                      ),
  ...
  void initState() {
      ...
      for(int i=1; i<paymentGetApiController.paymentgetwayapi!.paymentList!.length; i++){
        if(int.parse(paymentGetApiController.paymentgetwayapi!.defaultPayment.toString()) == paymentGetApiController.paymentgetwayapi!.paymentList![i].id){
          setState((){
            ...
            paymentname = paymentGetApiController.paymentgetwayapi!.paymentList![i].name!;
            print("+++++$payment");
            print("+++++$i");
          });
        }
    ...
    /// origin marker
    _addMarker11(LatLng(latitudepick, longitudepick), "origin", BitmapDescriptor.defaultMarker);

    /// destination marker
    _addMarker2(LatLng(latitudedrop, longitudedrop), "destination", BitmapDescriptor.defaultMarkerWithHue(90));

    for(int a=0; a<_dropOffPoints.length; a++){
      _addMarker3("destination");
    }

    getDirections11(lat1: PointLatLng(latitudepick, longitudepick),lat2: PointLatLng(latitudedrop, longitudedrop),dropOffPoints: _dropOffPoints);
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      picksubtitle == "" ? const SizedBox() :  Text(
                        picksubtitle,
                        maxLines: 3,
                        style: const TextStyle(
                          color: Colors.grey,
  ...
  Future _addMarker2(LatLng position, String id, BitmapDescriptor descriptor) async {
  ...
  _addMarker3(String id) async {
    for (int a = 0; a < _dropOffPoints.length; a++) {
      ...
      LatLng position = LatLng(_dropOffPoints[a].latitude, _dropOffPoints[a].longitude);
  ...
  Future getDirections11({required PointLatLng lat1,required PointLatLng lat2,required List<PointLatLng> dropOffPoints}) async {
  ...
  void _addMarkers() async {
    ...
    Future<BitmapDescriptor> _loadIcon(String url, {int targetWidth = 30, int targetHeight = 50}) async {
      try {
        if (url.isEmpty || url.contains("undefined")) {
          // Fallback to a default icon if the URL is invalid
          return BitmapDescriptor.defaultMarker;
    ...
    setState(() {
      ...
      for (var i = 0; i < vihicallocations.length; i++) {
        ...
        MarkerId markerId = MarkerId("${homeMapController.homeMapApiModel!.list![i].id}");
        Marker marker = Marker(
          markerId: markerId,
          icon: _icons[i],
          position: LatLng(double.parse(homeMapController.homeMapApiModel!.list![i].latitude.toString()),double.parse(homeMapController.homeMapApiModel!.list![i].longitude.toString())),
  ...
  void _addMarkers2() async {
    ...
    setState(() {
      ...
      for (var i = 0; i < vihicallocationsbiddingon.length; i++) {
        ...
        final markerId = MarkerId('${homeMapController.homeMapApiModel!.list![i].id}');
        final marker = Marker(
          markerId: markerId,
          // position: vihicallocationsbiddingon[i],
          position: LatLng(double.parse(homeMapController.homeMapApiModel!.list![i].latitude.toString()),double.parse(homeMapController.homeMapApiModel!.list![i].longitude.toString())),
  ...
  void _navigateAndRefresh() async {
    final result = await
    Navigator.push(context, MaterialPageRoute(builder: (context) => PickupDropPoint(pagestate: false,bidding: homeApiController.homeapimodel!.categoryList![select1].bidding.toString(),)));
    if (result != null && result.shouldRefresh) { _refreshPage(); }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: GetBuilder<HomeApiController>(builder: (homeApiController) {
        return homeApiController.isLoading ? Center(child: CircularProgressIndicator(color: theamcolore,)) : Stack(
          children: [
            ...
            lathome == null ? Center(child: CircularProgressIndicator(color: theamcolore,)) : GoogleMap(
              gestureRecognizers: {
                Factory<OneSequenceGestureRecognizer>(() => EagerGestureRecognizer())
              },
              initialCameraPosition:  pickupcontroller.text.isEmpty || dropcontroller.text.isEmpty ? CameraPosition(target: LatLng(lathome,longhome), zoom: 13) : CameraPosition(target: LatLng(latitudepick,longitudepick), zoom: 13),
              // initialCameraPosition:  CameraPosition(target: LatLng(21.2408,72.8806), zoom: 13),
              mapType: MapType.normal,
              // markers: markers.,
              markers: pickupcontroller.text.isEmpty || dropcontroller.text.isEmpty ? Set<Marker>.of(markers.values) : Set<Marker>.of(markers11.values),
                  ...
                  Expanded(
                    ...
                    child: InkWell(
                      ...
                      child: Container(
                        ...
                        child: Center(child: Image(image: const AssetImage("assets/menu.png"),height: 20,color: notifier.textColor,)),
                      ),
                    ),
                  ),
                  Expanded(
                    ...
                    child: Container(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                            ...
                            border: OutlineInputBorder(borderRadius: BorderRadius.all(Radius.circular(25)),borderSide: BorderSide(color: notifier.background)),
                            enabledBorder:  OutlineInputBorder(borderRadius: BorderRadius.all(Radius.circular(25)),borderSide: BorderSide(color: notifier.background)),
                            hintText: "Your Current Location".tr,
                            hintStyle: const TextStyle(color: Colors.grey,fontFamily: "SofiaProBold",fontSize: 14),
                            focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.all(Radius.circular(25)),borderSide: BorderSide(color: notifier.textColor)),
                            prefixIcon: Padding(
                            ...
                            suffixIcon:  InkWell(
                                ...
                                child: SvgPicture.asset("assets/svgpicture/gpsicon.svg",))
                        ),
                      ),
                    ),
                  )
            ...
            DraggableScrollableSheet(
              ...
              initialChildSize: 0.45, // Set the default height to 50% of the screen
                    ...
                    slivers: [
                      ...
                      SliverList.list(
                          children: [
                                    ...
                                    return InkWell(
                                      onTap: () {
                                        setState(() {
                                          ...
                                          mid = homeApiController.homeapimodel!.categoryList![index].id.toString();
                                          mroal = homeApiController.homeapimodel!.categoryList![index].role.toString();
                                          print("*****mid*-**:::::------$mid");
                                          _iconPaths.clear();
                                          vihicallocations.clear();
                                          ...
                                          pickupcontroller.text.isEmpty || dropcontroller.text.isEmpty ? markers.clear() : "";
                                          pickupcontroller.text.isEmpty || dropcontroller.text.isEmpty ? fun().then((value) {
                                            setState(() {
                                            });
                                            getCurrentLatAndLong(lathome, longhome);
                                            // _loadMapStyles();
                                          ...
                                          calculateController.calculateApi(context: context,uid: userid.toString(),mid: mid, mrole: mroal, pickup_lat_lon: "$latitudepick,$longitudepick", drop_lat_lon: "$latitudedrop,$longitudedrop", drop_lat_lon_list: onlypass).then((value) {
                                      ...
                                      child: Padding(
                                        ...
                                        child: Container(
                                          ...
                                          // width: 100,
                                          decoration: BoxDecoration(
                                            color: select1 == index ? theamcolore.withOpacity(0.08) : notifier.containercolore,
                                            borderRadius: BorderRadius.circular(10),
                                          ),
                                          child: Padding(
                                            padding:  const EdgeInsets.all(8.0),
                                            child: Column(
                                              crossAxisAlignment: select1 == index ? CrossAxisAlignment.start : CrossAxisAlignment.center,
                                              children: [
                                                Row(
                                                  crossAxisAlignment: CrossAxisAlignment.start,
                                                  mainAxisAlignment: MainAxisAlignment.start,
                                                  children: [
                                                    SizedBox(
                                                        height: 40,
                                                        width: 40,
                                                        child: Image(image: NetworkImage("${Config.imageurl}${homeApiController.homeapimodel!.categoryList![index].image}"),height: 40,),
                                                    ),
                                                    select1 == index ? const SizedBox(width: 5,) : const SizedBox(),
                                                    select1 == index ? InkWell(
                                                        onTap: () {
                                                          setState(() {
                                                            vihicalInformationApiController.vihicalinformationApi(vehicle_id: homeApiController.homeapimodel!.categoryList![index].id.toString()).then((value) {
                                                              Get.bottomSheet(
                                                                    ...
                                                                    return Container(
                                                                      ...
                                                                      child: Padding(
                                                                        ...
                                                                        child: Column(
                                                                          ...
                                                                          mainAxisAlignment: MainAxisAlignment.start,
                                                                          children: [
                                                                            Image.network("${Config.imageurl}${vihicalInformationApiController.vihicalInFormationApiModel!.vehicle!.image}",height: 60,),
                                                                            Text("${vihicalInformationApiController.vihicalInFormationApiModel!.vehicle!.name}",style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold,color: notifier.textColor),),
                                                                            const SizedBox(height: 10,),
                                                                            Text("${vihicalInformationApiController.vihicalInFormationApiModel!.vehicle!.description}",style: TextStyle(fontSize: 16,color: notifier.textColor),),
                                                                            const SizedBox(height: 20,),
                                                                            CommonOutLineButton(bordercolore: theamcolore, onPressed1: () {Get.back();},txt1: "Close".tr,context: context)
                                                                          ],
                                                                        ),
                                                                      ),
                                                                    );
                                                        ...
                                                        child: Image(image: const AssetImage("assets/info-circle.png"),height: 20,color: theamcolore,)) : const SizedBox(),
                                                  ],
                                                ),
                                                const SizedBox(height: 10,),
                                                Text("${homeApiController.homeapimodel!.categoryList![index].name}",style: TextStyle(color: notifier.textColor),),
                                              ],
                                            ),
                                          ),
                                        ),
                                  ...
                                  return Container(
                                   ...
                                   decoration: const BoxDecoration(
                                    ...
                                    child: Column(
                                      children: [
                                        // _navigateAndRefresh();
                                        Expanded(child: PickupDropPoint(pagestate: false,bidding: homeApiController.homeapimodel!.categoryList![select1].bidding.toString(),)),
                                      ],
                                    ),
                                  );
                                },);
                              ...
                              child: Padding(
                                ...
                                child: Container(
                                  ...
                                  child: ListTile(
                                    ...
                                    title: pickupcontroller.text.isEmpty ? Transform.translate(offset: const Offset(-20, 0),child: Text("${addresshome ?? "Searching for you on the map...".tr}",style: TextStyle(color: notifier.textColor),)) :
                                    Transform.translate(offset: const Offset(-20, 0),child: Text(pickupcontroller.text,style: TextStyle(color: notifier.textColor),)),
                                    // trailing: InkWell(
                                    //   onTap: () {
                            ...
                            textfieldlist.length > 0 ? const SizedBox(height: 0) : const SizedBox(height: 10),
                            textfieldlist.length > 0 ?  InkWell(
                              onTap: () {
                                // _navigateAndRefresh();
                                showModalBottomSheet(
                                  isScrollControlled: true,
                              ...
                              child: Padding(
                                ...
                                child: Container(
                                  ...
                                  child: ListTile(
                                    ...
                                    title: Transform.translate(offset: const Offset(-20, 0),child: Text("${textfieldlist.length + 1} route stops",style: TextStyle(color: notifier.textColor),)),
                                    trailing: Icon(Icons.add,color: notifier.textColor,),
                                  ),
                                ),
                              ),
                            ) :
                                ...
                                child: Container(
                                  ...
                                  child:  Padding(
                                    ...
                                    child: Row(
                                      ...
                                      // mainAxisAlignment: MainAxisAlignment.start,
                                      children: [
                                        Image(image: const AssetImage("assets/search.png"),height: 20,color: notifier.textColor,),
                                        const SizedBox(width: 15,),
                                        Flexible(
                                          child: Padding(
                                            padding: const EdgeInsets.only(top: 2),
                                            child: dropcontroller.text.isEmpty ?  Text("To",style: TextStyle(color: notifier.textColor,fontSize: 16),) : Text(dropcontroller.text,style: TextStyle(color: notifier.textColor,fontSize: 16),maxLines: 2,),
                                          ),
                                        ),
                                      ],
                                    ),
                            ...
                            homeApiController.homeapimodel!.categoryList![select1].bidding == "1" ? const SizedBox(height: 10,) : const SizedBox(),
                            homeApiController.homeapimodel!.categoryList![select1].bidding == "1" ?  Padding(
                              padding: const EdgeInsets.only(left: 20,right: 20),
                              child: InkWell(
                                onTap: () {
                                ...
                                child: Container(
                                  ...
                                  child: Padding(
                                    ...
                                    child: Row(
                                      children: [
                                        (pickupcontroller.text.isEmpty || dropcontroller.text.isEmpty) ? Text("${currencyy ?? globalcurrency}",style: TextStyle(color: notifier.textColor,fontSize: 16),) : Text("${currencyy ?? globalcurrency} ${dropprice == null ? "" : dropprice}",style: TextStyle(color: notifier.textColor,fontSize: 16),),
                                        const SizedBox(width: 8,),
                                        Text("Offer your fare".tr,style: TextStyle(color: notifier.textColor,fontSize: 16),),
                                        const Spacer(),
                                      ],
                                    ),
                                  ),
                            ...
                            // const SizedBox(height: 20,),
                            pickupcontroller.text.isEmpty || dropcontroller.text.isEmpty ? Padding(
                              padding: const EdgeInsets.all(10),
                              child: CommonButton(containcolore: theamcolore.withOpacity(0.2), onPressed1: () {
                                // rateBottomSheet();
                                // rateBottomSheet();
                                // Navigator.push(context, MaterialPageRoute(builder: (context) => const DetailsScreen()));
                              },context: context,txt1: "Find a driver".tr),
                            ) : Padding(
                              padding: const EdgeInsets.all(10),
                              child: CommonButton(containcolore: theamcolore,

                                  onPressed1:  homeApiController.homeapimodel!.categoryList![select1].bidding == "1" ?
                                      (){
                                        // bottomshhetopen = true;
                                        Buttonpresebottomshhet();
                                      }
                                  :
                                    ...
                                    Navigator.push(
                                  context,
                                  MaterialPageRoute(builder: (context) => HomeScreen(latpic: latitudepick,longpic: longitudepick,latdrop: latitudedrop,longdrop: longitudedrop,destinationlat: destinationlat)),
                                );
                                    modual_calculateController.modualcalculateApi(context: context,uid: userid.toString(), mid: mid, mrole: mroal, pickup_lat_lon: "$latitudepick,$longitudepick", drop_lat_lon: "$latitudedrop,$longitudedrop", drop_lat_lon_list: onlypass).then((value) {
  ...
  Widget draweropen(context){
    ...
    return Drawer(
      ...
      child: GetBuilder<HomeApiController>(builder: (homeApiController) {
        return  SingleChildScrollView(
          child: Column(
            children: [
              const SizedBox(height: 60,),
              homeApiController.isLoading ? Center(child: CircularProgressIndicator(color: theamcolore,)) :  InkWell(
                onTap: () {
                ...
                child: Stack(
                  children: [
                    Container(
                      ...
                      child: decodeUid["profile_image"] == "" ? Center(child: Text("${decodeUid["name"][0]}",style: const TextStyle(color: Colors.white,fontSize: 25))) :
                  ...
                  child: Row(
                    ...
                    children: [
                      Padding(
                        ...
                        child: Text("${homeApiController.homeapimodel!.cusRating!.avgStar}",style: TextStyle(fontSize: 14,color: notifier.textColor),),
                      ),
                      const SizedBox(width: 5,),
                      SvgPicture.asset("assets/svgpicture/star-fill.svg",height: 12,)
                    ],
                  ),
                ),
              ),
              const SizedBox(height: 10,),
              Text(username == "" ? "" : username,style: TextStyle(color: notifier.textColor,fontWeight: FontWeight.bold,fontSize: 20),),
              const SizedBox(height: 5,),
              InkWell(
                  onTap: () {
                    Navigator.of(context).push(MaterialPageRoute(builder: (context) => const  ProfileScreen(),));
                  },
                  child: Text("View profile",style: TextStyle(color: theamcolore),)),
              const SizedBox(height: 5,),
                    ...
                    onTap: () {
                      switch(index){
                        case 0:
                          Get.back();
                        case 1:
                          Navigator.of(context).push(MaterialPageRoute(builder: (context) =>  const MyRideScreen(),));
                        case 2:
                        ...
                        case 5:
                          Navigator.of(context).push(MaterialPageRoute(builder: (context) =>  const ReferAndEarn(),));
                        case 6:
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          Row(
                            children: [
                              SvgPicture.asset("${drowerimage[index]}",height: 25,color: notifier.textColor,),
                              // const Image(image: AssetImage("assets/info-circle.png"),height: 25,),
                              const SizedBox(width: 15,),
                              Text("${drowertitle[index]}".tr,style: TextStyle(fontSize: 16,color: notifier.textColor),),
                              const Spacer(),
                              index == 8 ?  SizedBox(
                                height: 20,
                                width: 30,
              ...
              GetBuilder<pagelistApiController>(builder: (pagelistcontroller) {
                return pagelistcontroller.isLoading ? const SizedBox() : ListView.separated(
                    ...
                    itemCount: pagelistcontroller.pageListApiiimodel!.pagesList!.length,
                    itemBuilder: (BuildContext context, int index) {
                      return InkWell(
                        onTap: () {
                          Navigator.push(context, MaterialPageRoute(builder: (context) => Page_List_description(title: pagelistcontroller.pageListApiiimodel!.pagesList![index].title.toString(),description: pagelistcontroller.pageListApiiimodel!.pagesList![index].description.toString()),));
                        },
                        child: Padding(
                          padding: const EdgeInsets.only(left: 10,right: 10,top: 20),
                          child: Column(
                            children: [
                              Row(
                                children: [
                                  SvgPicture.asset("assets/svgpicture/pagelist.svg",height: 25,color: notifier.textColor,),
                                  // const Image(image: AssetImage("assets/svgpicture/pagelist.svg"),height: 25,),
                                  const SizedBox(width: 15,),
                                  Text("${pagelistcontroller.pageListApiiimodel!.pagesList![index].title}",style: TextStyle(fontSize: 16,color: notifier.textColor),),
                                  const Spacer(),
                                  // index == 3 ? Container(
                                  //    height: 8,
                                  //    width: 8,
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      Row(
                        children: [
                          SvgPicture.asset("assets/svgpicture/logout.svg",height: 25,color: Colors.red,),
                          const SizedBox(width: 15,),
                          Text("Log Out".tr,style: const TextStyle(fontSize: 16,color: Colors.red),),
                          const Spacer(),
                        ],
                      ),
                    ],
                            ...
                            children: <Widget>[
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  const SizedBox(width: 10,),
                                  ElevatedButton(
                                    style: ButtonStyle(fixedSize: const WidgetStatePropertyAll(Size(130, 40)),elevation: const WidgetStatePropertyAll(0),shape: WidgetStatePropertyAll(RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),backgroundColor:  WidgetStatePropertyAll(theamcolore)),
                                    onPressed: () => {
                                      // resetNew(),
                                      deleteAccount.deleteaccountApi(id: useridgloable.toString()).then((value) {
                  ...
                  child: Column(
                    ...
                    children: [
                      Padding(
                        ...
                        child: Column(
                          children: [
                            isanimation == false ? const SizedBox() : lottie.Lottie.asset("assets/lottie/loadding.json",height: 30),
                            // isanimation == false ? const SizedBox() : lottie.Lottie.asset("assets/lottie/loadding2.json"),
                            const SizedBox(height: 20,),
                            Text("Set your price".tr,style: TextStyle(color: notifier.textColor,fontWeight: FontWeight.bold,fontSize: 22),),
                            const SizedBox(height: 10,),
                             Text("💡Raise the fare, increase your chances.".tr,style: const TextStyle(color: Colors.grey)),
                                ...
                                InkWell(
                                  onTap: () {
                                    setState((){
                                      ...
                                      // if(dropprice >= minprice){
                                      //   dropprice -= 1;
                                      //   amountcontroller.text = dropprice.toString();
                                      // }
                                      if(double.parse(dropprice.toString()) > minprice){
                                        dropprice -= 1;
                                        amountcontroller.text = dropprice.toString();
                                  ...
                                  child: Container(
                                    ...
                                    child: Center(child: Image(image: AssetImage("assets/minus.png"),color: notifier.textColor,height: 20,),),
                                  ),
                                ),
                                const Spacer(),
                                Center(
                                  child: SizedBox(
                                    ...
                                    child:  TextField(
                                      ...
                                      style: TextStyle(fontSize: 30,color: notifier.textColor),
                                      // onChanged: (value) {
                                      //   // setState((){
                                      //   //   setState((){
                                      //   //     if(int.parse(amountcontroller.text) > maxprice){
                                      //   //       amountcontroller.clear();
                                      //   //       amountcontroller.text = maxprice.toString();
                                      //   //       toast = 1;
                                      //   //       print("fffff");
                                      //   //     }else if(int.parse(amountcontroller.text) < minprice){
                                      //   //       amountcontroller.clear();
                                      //   //       amountcontroller.text = minprice.toString();
                                      //   //       toast = 2;
                                      //   //       print("hhhhh");
                                      //   //     }else{
                                      //   //       toast = 0;
                                      //   //       // dropprice =
                                      //   //       dropprice = int.parse(amountcontroller.text);
                                      //   //     }
                                      //   //   });
                                      //   // });
                                      // },
                                ...
                                InkWell(
                                  onTap: () {
                                    setState((){
                                      ...
                                      if(double.parse(dropprice.toString()) < maxprice){
                                        // dropprice = amountcontroller.text;
                                        dropprice += 1;
                                        amountcontroller.text = dropprice.toString();
                                        mainamount = dropprice.toString();
                                  ...
                                  child: Container(
                                    ...
                                    child: Center(child: Image(image: AssetImage("assets/plus.png"),color: notifier.textColor,height: 20,),),
                                  ),
                                ),
                              ],
                            ),
                            const SizedBox(height: 20,),
                            toast == 1 ? Text("Maximum fare is $currencyy$maxprice",style: const TextStyle(color: Colors.red),) : toast == 2 ?  Text("Minimum fare is $currencyy$minprice",style: const TextStyle(color: Colors.red),) : const SizedBox(),
                            const SizedBox(height: 10,),
                            ListTile(
                              contentPadding: EdgeInsets.zero,
                              ...
                              // ),
                              leading: const Image(image: AssetImage("assets/automatically.png"),height: 30,width: 30,),
                              title: Text("Automatically book the nearest driver for (${globalcurrency}${amountcontroller.text})".tr,style: TextStyle(color: notifier.textColor,fontSize: 16),),
                              trailing: SizedBox(
                                height: 30,
                                width: 40,
                                child: Transform.scale(
                      ...
                      Container(
                        ...
                        decoration:  BoxDecoration(
                            ...
                            boxShadow: [
                              BoxShadow(color: Colors.grey,offset: Offset(0, -0.4),blurRadius: 5),
                            ]
                        ),
                        child: Padding(
                          padding: const EdgeInsets.only(left: 10,right: 10),
                          child: Column(
                            children: [
                                  ...
                                  Expanded(
                                    child: InkWell(
                                                    ...
                                                    child: Scaffold(
                                                      ...
                                                      floatingActionButton: Padding(
                                                        ...
                                                        CommonButton(containcolore: theamcolore, onPressed1: () {
                                                          ...
                                                          Buttonpresebottomshhet();
                                                          // Get.back();
                                                        },txt1: "CONTINUE".tr,context: context),
                                                      ),
                                                      body: Container(
                                                        // height: 450,
                                                        decoration: BoxDecoration(
                                                        ...
                                                        child:  Padding(
                                                          ...
                                                          child: Column(
                                                            ...
                                                            children: <Widget> [
                                                              ...
                                                              Text('Payment Getway Method'.tr,style: TextStyle(fontFamily: "SofiaProBold",fontSize: 18,color: notifier.textColor)),
                                                              const SizedBox(height: 10,),
                                                              walleteamount == 0 ? const SizedBox() : Row(
                                                                children: [
                                                                  SvgPicture.asset("assets/svgpicture/wallet.svg",height: 30,color: theamcolore,),
                                                                  const SizedBox(width: 10,),
                                                                  Text("My Wallet (${globalcurrency}${walleteamount})",style: TextStyle(color: notifier.textColor),),
                                                                  const Spacer(),
                                                                  Transform.scale(
                                                                    scale: 0.8,
                                                                    child: CupertinoSwitch(
                                                              ...
                                                              Expanded(
                                                                child: ListView.separated(
                                                                    ...
                                                                    shrinkWrap: true,
                                                                    scrollDirection: Axis.vertical,
                                                                    itemCount: paymentGetApiController.paymentgetwayapi!.paymentList!.length,
                                                                    itemBuilder: (BuildContext context, int index) {
                                                                      return InkWell(
                                                                        onTap: () {
                                                                          setState(() {
                                                                            // payment = index;
                                                                            payment = paymentGetApiController.paymentgetwayapi!.paymentList![index].id!;
                                                                            paymentname = paymentGetApiController.paymentgetwayapi!.paymentList![index].name!;
                                                                            // paymentmethodId = paymentGetApiController.paymentgetwayapi!.paymentdata[index].id;
                                                                          });
                                                                        },
                                                                        child: paymentGetApiController.paymentgetwayapi!.paymentList![index].status == "0" ? const SizedBox() :  Container(
                                                                          height: 90,
                                                                          ...
                                                                          child: Center(
                                                                            child:  ListTile(
                                                                              ...
                                                                              title: Padding(
                                                                                ...
                                                                                child: Text(paymentGetApiController.paymentgetwayapi!.paymentList![index].name.toString(),style: TextStyle(fontSize: 16,fontFamily: "SofiaProBold",color: notifier.textColor),maxLines: 2,),
                                                                              ),
                                                                              subtitle: Padding(
                                                                                padding: const EdgeInsets.only(bottom: 4),
                                                                                child: Text(paymentGetApiController.paymentgetwayapi!.paymentList![index].subTitle.toString(),style: TextStyle(fontSize: 12,fontFamily: "SofiaProBold",color: notifier.textColor),maxLines: 2,),
                                                                              ),
                                                                              trailing: Radio(
                                      ...
                                      child: ListTile(
                                        ...
                                        leading: const Image(image: AssetImage("assets/payment.png"),height: 30,width: 30,),
                                        title: Transform.translate(offset: const Offset(-15, 0),child: Text(paymentname,style: TextStyle(color: notifier.textColor),)),
                                        trailing: Image(image: AssetImage("assets/angle-right-small.png"),height: 30,color: notifier.textColor,),
                                      ),
                                    ),
                                  ),
                                    ...
                                    child: InkWell(
                                      onTap: () {
                                        {
                                          ...
                                          setState((){
                                            ...
                                            if(couponadd.isEmpty){
                                              for(int i=0; i<paymentGetApiController.paymentgetwayapi!.couponList!.length; i++){
                                                couponadd.add(false);
                                              }
                                            }
                                          });
                                                      ...
                                                      appBar: AppBar(
                                                        ...
                                                        leading: Padding(
                                                          ...
                                                          child: InkWell(
                                                              ...
                                                              child: Image(image: AssetImage("assets/arrow-left.png"),color: notifier.textColor,)),
                                                        ),
                                                        title: Transform.translate(
                                                            offset: const Offset(-10, 0),
                                                            child:  Padding(
                                                              padding: const EdgeInsets.only(top: 40),
                                                              child: Text("All coupons".tr,style: TextStyle(color: notifier.textColor,fontSize: 18),),
                                                            )),
                                                      ),
                                                      body: Padding(
                                                        padding: const EdgeInsets.all(15),
                                                        child: Column(
                                                          ...
                                                          children: [
                                                            Text("Best Coupon".tr,style: TextStyle(color: notifier.textColor,fontSize: 20),),
                                                            const SizedBox(height: 20,),
                                                            Expanded(
                                                              child: ListView.builder(
                                                                shrinkWrap: true,
                                                                itemCount: paymentGetApiController.paymentgetwayapi!.couponList!.length,
                                                                itemBuilder: (context, index) {
                                                                  return Container(
                                                                    // height: 230,
                                                                      ...
                                                                      child: Row(
                                                                        children: [
                                                                          Column(
                                                                            crossAxisAlignment: CrossAxisAlignment.start,
                                                                            children: [
                                                                              Text("${paymentGetApiController.paymentgetwayapi!.couponList![index].title}",style:  TextStyle(color: notifier.textColor,fontWeight: FontWeight.bold,fontSize: 18),),
                                                                              const SizedBox(height: 5,),
                                                                              Text("${paymentGetApiController.paymentgetwayapi!.couponList![index].subTitle}",style: const TextStyle(color: Colors.grey),),
                                                                              const SizedBox(height: 10,),
                                                                              Row(
                                                                                children: [
                                                                                   Text("Coupon Code: ".tr,style: TextStyle(color: notifier.textColor),),
                                                                                  Text("${paymentGetApiController.paymentgetwayapi!.couponList![index].code}",style: TextStyle(color: theamcolore,fontWeight: FontWeight.bold),),
                                                                                ],
                                                                              ),
                                                                              const SizedBox(height: 5,),
                                                                              Row(
                                                                                children: [
                                                                                   Text("Coupon Amount: ".tr,style: TextStyle(color: notifier.textColor),),
                                                                                  Text("${globalcurrency}${paymentGetApiController.paymentgetwayapi!.couponList![index].discountAmount}",style: TextStyle(fontWeight: FontWeight.bold,color: notifier.textColor),),
                                                                                ],
                                                                              ),
                                                                              const SizedBox(height: 5,),
                                                                              Row(
                                                                                children: [
                                                                                   Text("Minimum Amount: ".tr,style: TextStyle(color: notifier.textColor),),
                                                                                  Text("${globalcurrency}${paymentGetApiController.paymentgetwayapi!.couponList![index].minAmount}",style: TextStyle(fontWeight: FontWeight.bold,color: notifier.textColor),),
                                                                                ],
                                                                              ),
                                                                              const SizedBox(height: 5,),
                                                                              Row(
                                                                                children: [
                                                                                   Text("Ex Date: ".tr,style: TextStyle(color: notifier.textColor),),
                                                                                  Text(paymentGetApiController.paymentgetwayapi!.couponList![index].endDate.toString().split(" ").first,style: TextStyle(fontWeight: FontWeight.bold,color: notifier.textColor),),
                                                                                ],
                                                                              ),
                                                                              const SizedBox(height: 15),
                                                                              couponadd[index] == true ? InkWell(
                                                                                  onTap: () {
                                                                                    setState((){
                                                                                      // couponindex = index;
                                                                                  ...
                                                                                  child: Container(
                                                                                      ...
                                                                                      child: const Center(child: Text("Remove",style: TextStyle(color: Colors.red),)))) : InkWell(
                                                                                onTap: () {
                                                                                ...
                                                                                child: Container(
                                                                                    ...
                                                                                    child: Center(child: Text("Apply coupons".tr,style: TextStyle(color: int.parse(paymentGetApiController.paymentgetwayapi!.couponList![index].minAmount!) < int.parse(mainamount) ? theamcolore : Colors.grey),))
                                                                                ),
                                                                              ),
                                                                            ],
                                                                          ),
                                                                          ...
                                                                          Column(
                                                                            ...
                                                                            mainAxisAlignment: MainAxisAlignment.center,
                                                                            children: [
                                                                              SvgPicture.asset("assets/svgpicture/offerIcon.svg",height: 50,)
                                                                            ],
                                                                          ),
                                                                        ],
                                                                      ),
                                      ...
                                      child: ListTile(
                                        ...
                                        leading: const Padding(
                                          ...
                                          child: Image(image: AssetImage("assets/coupon.png"),height: 30,width: 30,),
                                        ),

                                        title: couponname == "" ? Transform.translate(offset: const Offset(-15, 10),child: Text("Coupon".tr,style: TextStyle(color: notifier.textColor),)) : Transform.translate(offset: const Offset(-15, 0),child: Text(couponname,style: TextStyle(color: notifier.textColor),)),
                                        subtitle: couponname == "" ? const Text("") : Transform.translate(offset: const Offset(-15, 0),child: Text("Coupon applied".tr,style: TextStyle(fontSize: 12,color: notifier.textColor),)),
                                        trailing: Image(image: AssetImage("assets/angle-right-small.png"),height: 30,color: notifier.textColor,),
                              ...
                              },context: context,txt1: "Book for $currencyy${amountcontroller.text}")
                                  :
                              AnimatedBuilder(
                                ...
                                builder: (context, child) {
                                  return SizedBox(
                                    ...
                                    child: ElevatedButton(
                                      style: ButtonStyle(padding: const WidgetStatePropertyAll(EdgeInsets.zero),
                                        elevation: const WidgetStatePropertyAll(0),
                                        overlayColor: const WidgetStatePropertyAll(Colors.transparent),
                                        backgroundColor: WidgetStatePropertyAll(theamcolore.withOpacity(0.4)),
                                          ...
                                          setState((){
                                            ...
                                            if(int.parse(amountcontroller.text) > maxprice){
                                              ...
                                              if(offerpluse == true){
                                                ...
                                                print("fgvjgfsvjhsgfvjhbfgvjhafgbvhjkafgv");
                                        ...
                                        children: [
                                          ClipRRect(
                                            ...
                                            LinearProgressIndicator(
                                              ...
                                              backgroundColor: theamcolore.withOpacity(0.1),
                                              color: theamcolore,
                                            ),
                                          ),
                                          offerpluse == true ? Text(
                              ...
                              },context: context,txt1: "Cancel Request".tr),
                            ],
                          ),
                        ),
                      ),
              ...
              Positioned(
                ...
                child: InkWell(
                  ...
                  child: Container(
                      ...
                      child: Center(child: Image(image: AssetImage("assets/arrow-left.png"),height: 20,color: notifier.textColor,))),
                ),
              )
            ],
          );
  ...
  orderfunction(){
    ...
    addVihicalCalculateController.addvihicalcalculateApi(
      pickupadd: {"title":"${picktitle == "" ? addresspickup : picktitle}","subt":picksubtitle},
...
class RefreshData { final bool shouldRefresh; RefreshData(this.shouldRefresh); }
