...
class Modual_CalculateController extends GetxController implements GetxService {
  ...
  Future  modualcalculateApi({context,required String uid,required String mid,required String mrole,required String pickup_lat_lon,required String drop_lat_lon,required List drop_lat_lon_list}) async {
    Map body = {
      "uid": uid,
      "mid": mid,
      "mrole": mrole,
    ...
    var response = await http.post(Uri.parse(Config.baseurl + Config.modualcalculate),
    ...
    if (response.statusCode == 200) {
      if (data["Result"] == true) {
        modualCalculateApiModel = modualCalculateApiModelFromJson(response.body);
