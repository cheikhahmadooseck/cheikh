...
class MyRideDetailApiController extends GetxController implements GetxService {
  ...
  Future mydetailApi({context,required String uid,required String request_id,required String status}) async {
    Map body = {
      "uid" : uid,
    ...
    var response = await http.post(Uri.parse(Config.baseurl + Config.myridedetail),
        body: jsonEncode(body), headers: userHeader);

    print('+ + + + + MyRideDetailApiController + + + + + + :--- $body');
    print('- - - - - MyRideDetailApiController - - - - - - :--- ${response.body}');
