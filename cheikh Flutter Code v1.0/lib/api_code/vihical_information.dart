...
class VihicalInformationApiController extends GetxController implements GetxService {
  ...
  Future vihicalinformationApi({context,required String vehicle_id}) async {
    ...
    var response = await http.post(Uri.parse(Config.baseurl + Config.vihicalinformation),
        body: jsonEncode(body), headers: userHeader);

    print('+ + + + + VihicalDetailApiController + + + + + + :--- $body');
    print('- - - - - VihicalDetailApiController - - - - - - :--- ${response.body}');

    var data = jsonDecode(response.body);

    if (response.statusCode == 200) {
      if (data["Result"] == true) {
        vihicalInFormationApiModel = vihicalInFormationApiModelFromJson(response.body);
