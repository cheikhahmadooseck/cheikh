...
class AllRequestDataApiController extends GetxController implements GetxService {
  ...
  Future allrequestApi({context,required String uid,required String status}) async {
    Map body = {
      "uid": uid,
      "status": status,
    };
    ...
    var response = await http.post(Uri.parse(Config.baseurl + Config.allrequest),
    ...
    if (response.statusCode == 200) {
      if (data["Result"] == true) {
        ...
        if(status == "upcoming"){
          print("+++++ upcoming +++++");
          allRequestDataModelupcoming = allRequestDataModelFromJson(response.body);
        }else if(status == "completed"){
          print("+++++ completed +++++");
          allRequestDataModelcompleted = allRequestDataModelFromJson(response.body);
        }else{
          print("+++++ cancelled +++++");
          allRequestDataModelcancelled = allRequestDataModelFromJson(response.body);
        }
