...
class VihicalRideCompleteOrderApiController extends GetxController implements GetxService {

  VihicalRideCompleteApiModel? vihicalRideCompleteApiModel;
  bool orederloader = false;
  bool isloadoing = false;

  Future vihicalridecomplete({required String payment_id,required String payment_img,required String uid, required String d_id, required String request_id, required String wallet,context}) async {
    ...
    var request = http.MultipartRequest('POST', Uri.parse(Config.baseurl + Config.vihicalridecomplete));
    ...
    if(payment_img != ""){
      request.files.add(await http.MultipartFile.fromPath('payment_img', payment_img));
    }else{
      print("ffffffffffff");
    }
    ...
    if (response.statusCode == 200) {
      ...
      if(responsnessaj["Result"] == true){
        ...
        vihicalRideCompleteApiModel= VihicalRideCompleteApiModel.fromJson(responsnessaj);
